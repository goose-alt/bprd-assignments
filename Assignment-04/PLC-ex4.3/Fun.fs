...
let rec eval (e : expr) (env : value env) : int =
    ...
    | Call(Var f, eArgs) -> 
      ...
      | Closure (f, xs, fBody, fDeclEnv) ->
        let combinedArguments = List.zip xs eArgs
        let fBodyEnv = List.fold (fun acc (varName, varValue) -> (varName, Int(eval varValue env)) :: acc) fDeclEnv combinedArguments 
        let fBodyEnvX = (f, fClosure) :: fBodyEnv
        eval fBody fBodyEnvX
